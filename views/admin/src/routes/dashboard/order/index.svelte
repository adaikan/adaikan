...
<script lang="ts">
	...
	let fake = Array(6);
	let orders: ClientApi.Admin.Order[] = [];
	...
	async function init() {
		try {
			await client.ready;
			orders = await client.admin.orders();
			fake = Array(0);
		} catch (error: any) {
		} finally {
			progress.hiding();
							...
							>
								...
								<div class="flex flex-[30%]">
									<div class="text-sm trunc-2">
										{order.item.map((item) => item.product.name).join(', ')}
									</div>
								</div>
								...
								<div class="flex flex-[10%] flex-col justify-between">
									...
									<div class="text-sm">Rp. {Currency.toMoney(order.delivery.cost + '')}</div>
								</div>
								<div class="flex flex-[12.5%]">
