...
<script lang="ts">
	...
	onMount(async () => {
		try {
			...
			slide = await client.admin.getSlide();
			profile = {
	...
	function inputFile(this: HTMLInputElement) {
		if (this.files) {
			...
			for (let index = 0; index < this.files.length; index++) {
				...
				slide.push(URL.createObjectURL(file));
			}
			slide = slide;
		}
	}
					...
					<div class="grid gap-4">
						{#if config}
							...
							<div class="form-control">
								...
								<div
									class="px-3 py-2 border border-primary bg-base-300 rounded-md overflow-auto flex gap-4"
								>
									<input
										on:input={inputFile}
										id="image"
										...
										accept="image"
										multiple
										class="opacity-0 w-full absolute"
									/>
									{#each slide as src}
										<img {src} alt="" class="w-8 h-8 object-cover object-center" />
									{/each}
									{#if slide.length == 0}
										<svg
											class="w-8 h-8"
