...
<script lang="ts">
	...
	let contact: SellerClientApi.Chat.Channel | undefined;
	...
	async function init() {
		try {
			...
			if (!store.chatNodeId) {
				...
				store.chatNodeId = contact.id;
				await client.store.update({
					where: { id: seller.id },
					data: { chatNodeId: store.chatNodeId },
				});
				snackbar.setText('Berhasil membuat kontak');
				snackbar.show();
			}
			await client.chat.connectContact({ myId: store.chatNodeId, theirId: id });
