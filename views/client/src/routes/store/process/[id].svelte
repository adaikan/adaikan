...
<script lang="ts">
	...
	let courier: SellerClientApi.Courier | undefined | null;
	...
	async function init() {
		try {
			...
			order = await sellerClient.order.search({
				where: {
					AND: [{ id: +$page.params.id }, { status: 'Process' }],
				},
				include: {
					buyer: true,
					item: { include: { product: true } },
					delivery: { include: { recipient: true, sender: true } },
				},
			...
			const { courier: courierId } = $session;
			if (!courierId) {
				return;
			} else {
				$session = {};
			}
			courier = await courierClient.init().courier.search({
				where: { id: courierId },
			});
	...
	async function accept() {
		try {
			loader.loading();
			if (!courier) {
				throw new Error('Kurir belum ada');
			}
					...
					delivery: {
						update: {
							courier: {
								...
								update: { contracted: true },
							},
						},
					},
				},
			});
			snackbar.setText('Pesanan mulai di kirim');
			snackbar.show();
				...
				{#if order}
					...
					<fieldset class="card div p-16 white">
						...
						<section class="section">
							...
							>
							<TextField
								value="{order.delivery.sender.telp}"
								readonly
								placeholder="-">Phone Number</TextField
							>
							<Textarea
					...
					<fieldset class="card div p-16 white">
						...
						{#if courier}
							<section class="section">
								<TextField value="{courier.name ?? ''}" readonly placeholder="-"
									>Nama</TextField
								>
								<TextField value="{courier.telp ?? ''}" readonly placeholder="-"
						...
						{:else}
							<section class="section">
								<Button class="primary-color" block on:click="{searchCourier}"
									>Cari Kurir</Button
								>
							</section>
						{/if}
		...
		<Footer class="white elevation-5">
			<div class="btn">
				{#if order}
					...
					<Button class="primary-color" on:click="{accept}">Kirim</Button>
					<Button
