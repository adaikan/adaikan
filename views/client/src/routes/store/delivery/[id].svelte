...
<script lang="ts">
	...
	async function init() {
		try {
			...
			order = await sellerClient.order.search({
				where: {
					AND: [{ id: +$page.params.id }, { status: 'Delivery' }],
				},
				include: {
					buyer: true,
	...
	async function accept() {
		try {
			loader.loading();
			if (order.delivery.status != 'Done') {
				throw new Error('Pengiriman belum selesai');
			}
			await sellerClient.order.update({
				...
				data: {
					...
					delivery: {
						update: {
							courier: {
								update: { contracted: false },
							},
						},
					},
				},
			});
			snackbar.setText('Pesanan Sampai');
			snackbar.show();
		...
		<Footer class="white elevation-5">
			<div class="btn">
				{#if order}
					...
					<Button class="primary-color" on:click="{accept}">Konfirmasi</Button>
					<Button
