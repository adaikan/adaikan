...
<script lang="ts">
	...
	let data: SellerClientApi.Data.Store;
	...
	async function init() {
		try {
			data = JSON.parse(sessionStorage.getItem('store') ?? 'null');
			if (!data) {
				throw new Error('Alamat tidak terjangkau');
			}

			const item: Item | null = JSON.parse(
				sessionStorage.getItem('store/map') ?? 'null'
			);
			...
			if (item) {
				...
				sessionStorage.removeItem('store/map');
			}
	...
	async function submit() {
		try {
			...
			await goto('/store/detail/', { replaceState: true });
		} catch (error: any) {
		} finally {
			loaded();
		}
		...
		<main class="main">
			<form id="pin" on:submit|preventDefault="{submit}">
				{#if data}
					...
					<div class="field">
						<Textarea value="{data.address}" autogrow rows="{3}" outlined readonly
							>Alamat</Textarea>
						<!-- <Textarea bind:value="{data.detail}" autogrow rows="{1}" outlined
							>Detail</Textarea> -->
					</div>
				{:else}
